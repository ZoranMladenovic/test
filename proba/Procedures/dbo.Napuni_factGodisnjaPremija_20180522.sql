...
GO
CREATE procedure [dbo].[Napuni_factGodisnjaPremija_20180522]
as
...
update SSIS_Polise 
SET ZacDat_Korigovani = DATEADD(DAY, 1, Zac_Dat), GodDat_Korigovani=DATEADD(DAY,1,Kon_Dat)
...
update SSIS_Polise 
SET ZacDat_Korigovani = DATEADD(DAY, 1, Zac_Dat), GodDat_Korigovani=Kon_Dat
WHERE ZacDat_Korigovani is NULL
	and day(Zac_Dat)=DAY(Kon_Dat) and MONTH(Zac_Dat)=MONTH(Kon_Dat) and YEAR(Zac_Dat)<YEAR(Kon_Dat) 

update SSIS_Polise 
SET ZacDat_Korigovani = Zac_Dat, GodDat_Korigovani=Kon_Dat
WHERE ZacDat_Korigovani is NULL


update SSIS_Polise SET KrajDat=case when Kon_Dat<isnull(Storno_dat,Kon_Dat) THEN Kon_Dat else isnull(Storno_dat,Kon_Dat) end, GodDat_Korigovani=DATEADD(YEAR,20,GodDat_Korigovani)
...
select d.factPolisa,  CASE WHEN  dateadd(YEAR,-b.broj, a.GodDat) <a.Zac_Dat THEN a.Zac_Dat ELSE dateadd(YEAR,-b.broj, a.GodDat) end pocetak,  
	CASE WHEN  dateadd(YEAR,-b.broj, a.GodDat_Korigovani) <a.ZacDat_Korigovani THEN a.ZacDat_Korigovani ELSE dateadd(YEAR,-b.broj, a.GodDat_Korigovani) end pocetakKorigovan,  
	case when dateadd(YEAR,-b.broj+1, a.GodDat_Korigovani) > a.KrajDat THEN a.KrajDat else dateadd(YEAR,-b.broj+1, a.GodDat_Korigovani) END kraj,
	--dateadd(day,-1,case when dateadd(YEAR,-b.broj+1, a.GodDat_Korigovani) > a.KrajDat THEN a.KrajDat else dateadd(YEAR,-b.broj+1, a.GodDat_Korigovani) END) kraj,
	0 as pocetakI, 0 as KrajI, 0 pocetakKorigovanI--, d.dimDinamika, d.VaziOd, case WHEN d.StornoDat<d.VaziDo THEN d.StornoDat else d.VaziDo end VaziDo
INTO #t
from SSIS_Polise a inner JOIN (select top 100 
		ROW_NUMBER() OVER(ORDER BY object_id) broj
		from sys.columns) b on dateadd(YEAR,-b.broj, a.GodDat_Korigovani) BETWEEN a.ZacDat_Korigovani and dateadd(day,-1,a.KrajDat)
...
INSERT #t
select d.factPolisa, a.Zac_Dat pocetak,  a.ZacDat_Korigovani pocetakKorigovan,  a.KrajDat kraj,	0 as pocetakI, 0 as KrajI, 0 as pocetakKorigovanI
from SSIS_Polise a 
	inner JOIN SSIS_Produkti c on a.Produkt_ID=c.Produkt_Id and c.Nadredjena <>5
	INNER JOIN factPolise d ON a.Poistka_ID=d.Polisa
WHERE d.dimDinamika=@Jedn
...
UPDATE a
SET pocetaki=d1.DimDatum, kraji=d2.DimDatum, pocetakKorigovanI=d3.DimDatum
from #t a INNER JOIN dimDatumi d1 ON a.pocetak=d1.Datum
	INNER JOIN dimDatumi d2 on a.kraj=d2.Datum
	INNER JOIN dimDatumi d3 on a.pocetakKorigovan=d3.Datum
...
from #t a INNER JOIN factPropisi b on a.factpolisa=b.factPolisa and b.DatOd BETWEEN a.pocetakKorigovanI and kraji
...
-- korekcija za rizike koji su jednokratni, a polisa nije - proveriti
Update a 
...
-- korekcija za tarife koje su jednokratne, a polisa nije - proveriti
Update a 
SET IznosKorigovan=c.IznosKorigovan
